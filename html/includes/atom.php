...
class Atom extends LocalObject
{
	...
	{		
		$query = "INSERT INTO ATOMS (ID,ID_ELEMENT,IONIZATION,[DESCRIPTION]) SELECT MAX(ID)+1,".$element_id.",(SELECT MAX(IONIZATION)+1 FROM ATOMS WHERE ID_ELEMENT=".$element_id."), (SELECT MAX(ID) FROM INTERFACE_CONTENT WHERE [TYPE]='atom_description') FROM ATOMS
...
";
	...
	{	
		$query = "DELETE FROM ATOMS WHERE ID=".$atom_id." SELECT TOP 1 ID FROM ATOMS WHERE ID_ELEMENT=".$element_id." ORDER BY IONIZATION DESC";
		$this->LoadFromSQL($query);		
	}
