...
class Atom extends LocalObject
{
	...
	{		
		if (ISLOCAL) $query = "SELECT SPECTRUM_IMG, MASS_NUMBER, IONIZATION,Z,ATOM_MASS,IONIZATION_POTENCIAL,ABBR,TABLEPERIOD,TABLEGROUP,NAME_EN,NAME_RU,NAME_RU_ALT,PERIODICTABLE.[TYPE] AS ELEMENT_TYPE,PERIODICTABLE.ID AS ELEMENT_ID, INTERFACE_CONTENT.* FROM  ATOMS,PERIODICTABLE,INTERFACE_CONTENT WHERE ATOMS.ID='$element_id' AND ATOMS.ID_ELEMENT = PERIODICTABLE.ID AND INTERFACE_CONTENT.ID=ATOMS.DESCRIPTION";
		else 		 $query = "SELECT SPECTRUM_IMG, MASS_NUMBER, IONIZATION,Z,ATOM_MASS,IONIZATION_POTENCIAL,ABBR,TABLEPERIOD,TABLEGROUP,NAME_EN,NAME_RU,NAME_RU_ALT,PERIODICTABLE.[TYPE] AS ELEMENT_TYPE,PERIODICTABLE.ID AS ELEMENT_ID, LIMITS, BREAKS, INTERFACE_CONTENT.* FROM  ATOMS,PERIODICTABLE,INTERFACE_CONTENT WHERE ATOMS.ID='$element_id' AND ATOMS.ID_ELEMENT = PERIODICTABLE.ID AND INTERFACE_CONTENT.ID=ATOMS.DESCRIPTION";
...
";
	...
	$query = "UPDATE ATOMS SET [MASS_NUMBER] = ".$post['atomMassNumber'].",[IONIZATION] = ".$post['atomIonization'].",[IONIZATION_POTENCIAL] = '".$post['atomIonizationPotencial']."'  WHERE ID = ".$post['atom_id']." 
