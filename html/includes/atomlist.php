...
class AtomList extends LocalList
{
    ...
    function Load()
    {
        $query = "SELECT ATOMS.ID, ATOMS.IONIZATION, ATOMS.IONIZATION_POTENCIAL, ATOMS.MASS_NUMBER, PERIODICTABLE.NAME_EN, NAME_RU, PERIODICTABLE.ABBR FROM ATOMS JOIN PERIODICTABLE ON ATOMS.ID_ELEMENT=PERIODICTABLE.ID";
        $this->LoadFromSQL($query);
    }
