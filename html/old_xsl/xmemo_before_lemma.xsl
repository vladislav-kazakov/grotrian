...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
  ...
  <xsl:variable name="Ecm_row_w">
	...
	<xsl:template match="Diagram">
		<svg version="1.1" xmlns="http://www.w3.org/2000/svg"  xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMinYMin">
	...
	<!-- END for view level energy -->	
      ...
      <g class="Data">
        <xsl:attribute name="transform">
          <xsl:value-of select="concat(concat('translate(', $Ecm_row_w), ')')"/>
        </xsl:attribute>
      ...
      <g class="Eev" id="EeV">
        <xsl:attribute name="transform">
          <xsl:value-of select="concat(concat('translate(', $n_terms*$term_row_w), ')')"/>
        </xsl:attribute>
	...
	<xsl:if test="./level[1]/@energy &gt; $energy_limit">
      ...
      <xsl:text> </xsl:text>
		...
		<g class="levels">
			...
			<xsl:for-each select="./level">
				<line class="level" energy="{@energy}" onmouseover="mouse_on_level(evt)" onmouseout="mouse_out_level(evt)" config="{@config}" j="{@j}">
	...
	<xsl:template match="Lines">
		<g id="transitions">
			<xsl:for-each select="./line">
				<line class="transition" onmouseover="mouse_on_tr(evt)" onmouseout="mouse_out_tr(evt)">
					<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
