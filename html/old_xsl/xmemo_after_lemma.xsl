...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" 
	...
	<xsl:copy>
			...
			&lt;!--
			!!!	(c) Pozhidaeva Natasha, Multimedia center, Novosibirsk state University, 2007
			--&gt;
			</xsl:text>
			<style type="text/css">
        <![CDATA[
...
<xsl:text disable-output-escaping="yes">&lt;![CDATA[<![CDATA[
		...
		var table_width = ]]></xsl:text><xsl:value-of select="$t_width"/>;
		var table_height = <xsl:value-of select="$t_height"/>; 
		var term_row_w = <xsl:value-of select="//svg:g[@class='term']/svg:rect/@width"/>;
		var term_row_h = <xsl:value-of select="//svg:g[@class='term']/svg:rect/@height"/>;<xsl:text disable-output-escaping="yes"><![CDATA[
	...
	function compress_shift_data(curTerm, dx)
	{
		...
		e.setAttribute('transform', 'translate(' + table_width +')');
		// shift Name
	...
	{
		minWave = parseFloat(window.parent.waveMinVal); 
		maxWave = parseFloat(window.parent.waveMaxVal);
		minEnergy = parseFloat(window.parent.energyMinVal); 
		maxEnergy = parseFloat(window.parent.energyMaxVal);
				...
				if (!equal(transitions[i], transitions[i+dn]))
				{
					...
					{	// try to shift i+dn transition
						...
						{// both levels are long
							...
							{
								//if (curTr.getAttribute('wavelength') != "1166.76") //alert(curTr.getAttribute('dx'));
								set_invisible(curTr);
								transitions.splice(i+dn, 1);
							}
						} 
	...
	function mouse_on_level(evt)
	{
		var target = evt.getTarget();
		show_level_info(target);
	}
	function mouse_out_level(evt)
	{
		var target = evt.getTarget();
