	...
	<div class="newperiodic">
		...
		{#foreach from=$periodic_table key=myId item=element#}
			...
			{#if 57<$element.Z && $element.Z<72#}
				{#counter#}
				<div class="{#$element.TYPE#} pick" style="left:{#$i*25+$i+112#}px; margin-top:240px;"><a title="{#$element.name|capitalize#}" href="{#$element.ID#}" class="plink" ><input type="hidden" name="elemid" value="{#$element.ID#}" /><span class="pnum">{#$element.Z#}</span><span class="pname">{#$element.ABBR#}</span></a></div> 
			{#/if#}
			
			{#*<!-- Draw Actinide -->*#}
			{#if 89<$element.Z && $element.Z<104#}
				{#counter#}
				<div class="{#$element.TYPE#} pick" style="left:{#$i*25+$i-364+112#}px; margin-top:266px;"><a title="{#$element.name|capitalize#}" class="plink" ><input type="hidden" name="elemid" value="{#$element.ID#}" /><span class="pnum">{#$element.Z#}</span><span class="pname">{#$element.ABBR#}</span></a></div> 
			{#/if#} 
			
			{#*<!-- Draw Superactinide -->*#}
			{#if 120<$element.Z && $element.Z<127#}
				{#counter#}
				<div class="{#$element.TYPE#} pick" style="left:{#$i*25+$i-616#}px; margin-top:292px;"><a title="{#$element.name|capitalize#}" class="plink" ><input type="hidden" name="elemid" value="{#$element.ID#}" /><span class="pnum">{#$element.Z#}</span><span class="pname">{#$element.ABBR#}</span></a></div> 
			{#/if#}
			
			
		{#*<!-- Draw Tritium and Deiterium -->*#} 
			{#if $element.ID==2817#}
				<div class="{#$element.TYPE#} pick" style="left:468px; margin-top:240px;"><a title="{#$element.name|capitalize#}" href="{#$element.ID#}" class="plink" ><input type="hidden" name="elemid" value="{#$element.ID#}" /><span class="pname_head">{#$element.ABBR#}</span></a></div> 
			{#/if#} 
			
			{#if $element.ID==39762#}
				<div class="{#$element.TYPE#} pick" style="left:468px; margin-top:266px;"><a title="{#$element.name|capitalize#}" href="{#$element.ID#}" class="plink" ><input type="hidden" name="elemid" value="{#$element.ID#}" /><span class="pname_head">{#$element.ABBR#}</span></a></div> 
			{#/if#} 
		{#/foreach#}
	</div>			
</div>
