function  spectrum(spectr){
	...
	$("#scaleSlider").slider({
		...
		slide: function( event, ui ) {
			$("#scaleValue").text(ui.value);		// показываем изменения масштаба	
	...
	function drawspectralines(spectr,MinLength,MaxLength,Center,ScaleValue){
		...
		svg.rect(g,0, 0, (WidthX), 100,{fill: '#333'});					// рисуем подложку для линий
		
		//svg.text(g,0, 110, String(Min),{fontSize:10,fill: "rgb(255, 255, 255)"});
		//svg.text(g,930, 110, String(Max),{fontSize:10,fill: "rgb(255, 255, 255)"});	
	...
	function lineOver() { 
	    ...
	    $("#LengthValue").text($(this).attr('length'));
	} 
