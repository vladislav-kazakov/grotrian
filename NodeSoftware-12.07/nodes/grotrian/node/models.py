...
class PeriodicTable(Model):
    id = PositiveIntegerField(primary_key=True, db_column='ID', db_index=True)
    name = CharField(max_length=10, db_column=u'ABBR', db_index=True)
    ...
    class Meta:
        db_table = u'PERIODICTABLE'
    def __unicode__(self):
        return u'%s'%self.id


class Species(Model):
    id = PositiveIntegerField(primary_key=True, db_column=u'ID', db_index=True)
#    name = CharField(max_length=10, db_index=True)
    ion = CharField(max_length=50, null=True, blank=True, db_column=u'IONIZATION', db_index=True)
    periodic_table = ForeignKey(PeriodicTable, null=True, db_column=u'ID_ELEMENT')
...
#    isotope = PositiveSmallIntegerField(null=True, blank=True)
    ...
    def get_symbol(self):
        return PeriodicTable.objects.filter(pk__in=self.periodic_table)
...
class State(Model):
    id = PositiveIntegerField(primary_key=True, db_column=u'ID', db_index=True)
    species = ForeignKey(Species, db_column=u'ID_ATOM')
    energy = FloatField(null=True, blank=True, db_column=u'ENERGY', db_index=True)
    config = CharField(max_length=128, null=True, blank=True, db_column=u'CONFIG')
...
#    coupling = CharField(max_length=2, null=True,blank=True)
    termprefix = PositiveIntegerField(null=True, blank=True, db_column=u'TERMPREFIX')
    termfirstpart = CharField(max_length=50, null=True, blank=True, db_column=u'TERMFIRSTPART')
    termsecondpart = CharField(max_length=50, null=True, blank=True, db_column=u'TERMSECONDPART')
    termmultiply = PositiveIntegerField(null=True, blank=True, db_column=u'TERMMULTIPLY')
    
    j = CharField(max_length=10, db_column=u'J', null=True, blank=True)
...
#    jc = DecimalField(max_digits=3, decimal_places=1,db_column=u'Jc', null=True,blank=True)
    ...
    # half-time
    lifetime = FloatField(null=True, blank=True, db_column=u'LIFETIME')
...
class Transition(Model):
    id = PositiveIntegerField(primary_key=True, db_column=u'ID', db_index=True)
    upstate = ForeignKey(State,related_name='isupperstate_trans',db_column='ID_UPPER_LEVEL',null=True)
    lostate = ForeignKey(State,related_name='islowerstate_trans',db_column='ID_LOWER_LEVEL',null=True)

    wave = FloatField(db_index=True, null=True, db_column='WAVELENGTH')
    species = ForeignKey(Species,db_column='ID_ATOM')
