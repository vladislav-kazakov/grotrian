...
def getSpeciesWithStates(transs):
    ...
    for spec in species:
        ...
        spec_transitions = models.Transition.objects.select_related(depth=3).filter(id__in=transids).filter(species=spec)
        #spec_transitions = transs.filter(species=spec)
        #spec_transitions = models.Transition.objects.filter(id__in=transids).filter(species=spec)
